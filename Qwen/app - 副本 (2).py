        ...
        with self.lock:
            if filepath not in self.data_cache:
                try:
                    ...
                    excel_data = pd.read_excel(filepath, sheet_name=None, dtype=str, engine=engine,header=1)
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # 4. （关键）将原始文件名与存储文件名关联起来。
        # 这里示例使用一个全局字典，实际应用中应使用数据库。
        # 例如：file_mapping[unique_filename] = original_filename
        # 请根据您的项目结构选择合适的存储方式（如数据库、Redis、JSON文件）。
        ...
        try:
            ...
            return jsonify({
                'success': True, 
                'message': '文件上传成功',
                'stored_filename': unique_filename,  # 返回存储用的文件名
                'original_filename': original_filename  # 返回原始文件名供前端显示
            })
        except Exception as e:
            os.remove(filepath)
